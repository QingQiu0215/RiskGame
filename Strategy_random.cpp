...
class Observer {
	...
	string phaseName;
public:
	Observer(Subject4 *sub, string str) {
		model = sub;
		phaseName = str;
...
public:
	ReinforceObserver(Subject4 *sub, string str) : Observer(sub, str) {}
	void update() {
...
public:
	AttackObserver(Subject4 *sub, string str) : Observer(sub, str) {}
	void update() {
		// 6. "Pull" information of interest
...
public:
	FortifyObserver(Subject4 *sub, string str) : Observer(sub, str) {}
	void update() {
		// 6. "Pull" information of interest
...
void Strategy_random::execute(vector <Player*> allPlayers, Deck* deckPtr, int armyQty_startup, int playerNum)
{
	Subject4 subj;
...
void Strategy_random::reinforce(vector <Player*> allPlayers, Deck* deckPtr, int armyQty_startup, int playerNum)
{
...
void Strategy_random::makeArmyQty_numberOfCountry(vector <Player*> allPlayers, int playerNum)
{
	int countryNum = 0;
...
void Strategy_random::makeArmyQty_continentValue(vector <Player*> allPlayers, int playerNum)
{
...
void Strategy_random::makeArmyQty_exchangeCard(vector <Player*> allPlayers, int playerNum)
{
...
void Strategy_random::makeReinforcementArmyQty(vector <Player*> allPlayers, int playerNum)
{
...
void Strategy_random::distributeArmy_reinforcement(vector <Player*> allPlayers, int armyQty_startup, int playerNum)
{
	int countryNum = 0;
	for (int i = 0; i < (allPlayers[playerNum - 1]->getMapPtr()->getCountry()).size(); i++)
	{
	...
	int index_randomCountry = findCountryIndex_random(allPlayers, playerNum);
	(allPlayers[playerNum - 1]->getMapPtr()->getCountry())[index_randomCountry]->setNumberOfArmies((allPlayers[playerNum - 1]->
		getMapPtr()->getCountry())[index_randomCountry]->getNumOfArmies() + armyQtytoDistribute);

	allPlayers[playerNum - 1]->setReinforcementArmyQty(0);
	cout << "Player" << playerNum << " distributed armies for the specific country: " << endl;
	//displayMyCountry(allPlayers, playerNum);
}
int Strategy_random::findCountryIndex_random(vector <Player*> allPlayers, int playerNum)
{
	...
	while (found)
	{
		int index;
		index = rand() % (size);
		if ((allPlayers[playerNum - 1]->getMapPtr()->getCountry())[index]->getPlayerNum() == playerNum)
		{
			found = false;
			return index;
		}
...
string Strategy_random::findCountryName_largestArmyQty(vector <Player*> allPlayers, int playerNum)
{
...
int Strategy_random::findCountryIndex_largestArmyQty(vector <Player*> allPlayers, int playerNum)
{
	int size = allPlayers[playerNum - 1]->getMapPtr()->getTotalNumOfCountry();
	int maxQtyOfArmy = 0;
	for (int i = 0; i < size; i++)
...
void Strategy_random::displayMyCountry(vector <Player*> allPlayers, int playerNum)
{
...
void Strategy_random::attack(vector <Player*> allPlayers, Deck* deckPtr, int playerNum)
{
	cout << "Player" << playerNum << " attacks random time." << endl;
	srand((unsigned)time(NULL));
	int attack_time = 0;
	attack_time = rand() % 100;
	...
	else
	{
		...
		randomDefenderIndex = rand() % (allPlayers[playerNum - 1]->getTemp_neighborCountries().size());

			int index_defenderCountry = indexOfCountry(allPlayers, allPlayers[playerNum - 1]->getTemp_neighborCountries()[randomDefenderIndex], playerNum);
			...
			{
				defenderNum = playerNum_defenderCountry(allPlayers, allPlayers[playerNum - 1]->getTemp_neighborCountries()[randomDefenderIndex], playerNum);
				attackerRolling(allPlayers, attackerCountry, playerNum);
				defenderRolling(allPlayers, allPlayers[playerNum - 1]->getTemp_neighborCountries()[randomDefenderIndex], playerNum);
				compareDiceAndFight(allPlayers, attackerCountry, allPlayers[playerNum - 1]->getTemp_neighborCountries()[randomDefenderIndex], playerNum, defenderNum);
			}
	}
...
bool Strategy_random::checkValidCountry_attacker(vector <Player*> allPlayers, string countryName, int playerNum)
{
	int size = allPlayers[playerNum - 1]->getMapPtr()->getTotalNumOfCountry();
	for (int i = 0; i < size; i++)
	{
...
//displaying the attacker's neighbor countries(belong to other players)
void Strategy_random::displayNeighborCountries(vector <Player*> allPlayers, string theAttackerCountry, int playerNum)
{
	int index = findCountryIndex_largestArmyQty(allPlayers, playerNum);
	int size = allPlayers[playerNum - 1]->getMapPtr()->getTotalNumOfCountry();
...
int Strategy_random::indexOfCountry(vector <Player*> allPlayers, string countryName, int playerNum)
{
	int i;
	int size = allPlayers[playerNum - 1]->getMapPtr()->getTotalNumOfCountry();
	for (i = 0; i < size; i++)
...
bool Strategy_random::checkValidCountry_defender(vector <Player*> allPlayers, string countryName, int playerNum)
{
...
int Strategy_random::playerNum_defenderCountry(vector <Player*> allPlayers, string defenderCountry, int playerNum)
{
...
void Strategy_random::attackerRolling(vector <Player*> allPlayers, string theAttackerCountry, int playerNum)
{
...
void Strategy_random::defenderRolling(vector <Player*> allPlayers, string theDefenderCountry, int playerNum)
{
...
void Strategy_random::compareDiceAndFight(vector <Player*> allPlayers, string theAttackerCountry, string theDefenderCountry, int playerNum, int defenderNum)
{
...
bool Strategy_random::checkGameOver(vector <Player*> allPlayers, int playerNum)
{
	int size = allPlayers[playerNum - 1]->getMapPtr()->getTotalNumOfCountry();
...
void Strategy_random::fortify(vector <Player*> allPlayers, int playerNum)
{
	...
	{
		int indexOftargetCountry = findNeighborCountryIndex_random(allPlayers, playerNum);
...
int Strategy_random::findNeighborCountryIndex_random(vector <Player*> allPlayers, int playerNum)
{

	int size = allPlayers[playerNum - 1]->getTemp_neighborCountries_inFortification().size();
	srand((unsigned)time(NULL));
	int index_random = 0;
	index_random = rand() % size;
	string finalTargetCountryName;
    finalTargetCountryName = allPlayers[playerNum - 1]->getTemp_neighborCountries_inFortification()[index_random];
	int i;
...
void Strategy_random::displayNeighborCountries_inFortification(vector <Player*> allPlayers, string theAttackerCountry, int playerNum)
{
	int index;
	int size = allPlayers[playerNum - 1]->getMapPtr()->getTotalNumOfCountry();
	for (int i = 0; i < size; i++)
...
int Strategy_random::index_targetCountry(vector <Player*> allPlayers, string targetCountry, int playerNum)
{
	int size = allPlayers[playerNum - 1]->getMapPtr()->getCountry().size();
	int i;
	for (i = 0; i < size; i++)
...
int Strategy_random::index_sourceCountry(vector <Player*> allPlayers, string sourceCountry, int playerNum)
{
	int size = allPlayers[playerNum - 1]->getMapPtr()->getCountry().size();
	int i;
	for (i = 0; i < size; i++)
